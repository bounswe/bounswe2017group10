...
const CulturalHeritage = ({ withCloseButton=false ,returnTo, close, culturalHeritage, shouldTruncate = false, showCommentSummary = false, showComments = false, favorite, withLink = true, showFavorite = true, showAnnotations = false, annotations, showAnnotation, hideAnnotation }) => {
  const ch = (
    <Row className="whitebox">
      <Col xs="5">
        { culturalHeritage.images.length > 0 ? (
          <Image src={ culturalHeritage.images[0].url } annotations={ showAnnotations ? annotations : [] } showAnnotation={ showAnnotation } hideAnnotation={ hideAnnotation } />
          ) : (
          <span>No Image</span>
          )
        }
